"""
Deep learning models. PyTorch chosen as the single framework.

- ANN: feed-forward classifier on the same static feature set as the
  classical models, with batch norm, dropout, and early stopping.
- LSTM: sequence model over each customer's time-ordered purchase events
  (amount, gap-in-days, category-code) for purchase-timing prediction.
- Autoencoder: unsupervised anomaly detector on scaled spending features;
  reconstruction error above a validated threshold flags anomalies.
"""
...
def train_ann(
    ...
    """Train the ANN with early stopping on validation loss."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = ChurnANN(input_dim=X_train.shape[1], hidden_dims=list(hidden_dims), dropout=dropout).to(device)
...
class PurchaseSequenceDataset(Dataset):
    """Pads variable-length per-customer purchase sequences.

    Each sequence step is a feature vector: [amount, gap_days, category_code].
    """

    def __init__(self, sequences: List[np.ndarray], targets: np.ndarray, max_len: Optional[int] = None):
...
class PurchaseLSTM(nn.Module):
    """LSTM over time-ordered purchase events to predict next-purchase timing
    (regression target: days until next purchase)."""
...
class SpendingAutoencoder(nn.Module):
    """Unsupervised autoencoder on scaled spending-pattern features.
    Reconstruction error above a validated threshold flags anomalous
...
def train_autoencoder(
    ...
    """Trains the autoencoder, then calibrates an anomaly threshold as the
    95th percentile of validation-set reconstruction error.

    Returns (model, history, anomaly_threshold).
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    with torch.no_grad():
        val_recon = model(X_val_t)
        per_sample_error = ((val_recon - X_val_t) ** 2).mean(dim=1).cpu().numpy()
    threshold = float(np.percentile(per_sample_error, 95))

    return model, history, threshold


def compute_reconstruction_error(model: SpendingAutoencoder, X: np.ndarray) -> np.ndarray:
    model.eval()
    device = next(model.parameters()).device
    with torch.no_grad():
        X_t = torch.tensor(X, dtype=torch.float32).to(device)
        recon = model(X_t)
        return ((recon - X_t) ** 2).mean(dim=1).cpu().numpy()